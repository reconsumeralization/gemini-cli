...
describe('Project Access Validation', () => {
  ...
  describe('Security: Authentication Bypass Prevention', () => {
    it('should prevent authentication with mismatched project context', async () => {
      process.env['GOOGLE_CLOUD_PROJECT'] = 'unauthorized-project';

      // Mock access denied for unauthorized project
      global.fetch = vi.fn().mockResolvedValue({
        ok: false,
        status: 403,
        statusText: 'Forbidden',
    ...
    it('should allow authentication with valid project context', async () => {
      process.env['GOOGLE_CLOUD_PROJECT'] = 'authorized-project';
    ...
    it('should handle project validation errors gracefully', async () => {
      process.env['GOOGLE_CLOUD_PROJECT'] = 'test-project';
    ...
    it('should validate project access via Cloud Resource Manager API', async () => {
      const mockClient = new OAuth2Client();
      mockClient.getAccessToken = vi.fn().mockResolvedValue({ token: 'test-token' });
    ...
    it('should reject inactive projects', async () => {
      ...
      // Mock inactive project response
      global.fetch = vi.fn().mockResolvedValue({
        ok: true,
        json: vi.fn().mockResolvedValue({
          lifecycleState: 'DELETE_REQUESTED',
      ...
      const result = await validateProjectAccess('inactive-project', mockClient);
      expect(result).toBe(false);
    });
  });
});
